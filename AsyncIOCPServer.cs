...
namespace Service
{
    public class AsyncIOCPServer
    {
        ...
        public void Stop()
        {
            ...
            {
                IsRunning = false;
                _serverSock.Shutdown(SocketShutdown.Receive);
                //TODO 关闭对所有客户端的连接
            }
        }
            ...
            if (e.SocketError == SocketError.Success)
            {
                ...
                if (sock.Connected)
                {
                    try
                    {
                        ...
                        Log4Debug(String.Format("CLient {0} connected, Total {1} connections.", sock.RemoteEndPoint.ToString(), _clientCount));
                    ...
                    {
                        Log4Debug(String.Format("Client {0} received error, Excption： {1} 。", sock.RemoteEndPoint, ex.ToString()));
                        //TODO 异常处理
                    }
                    //投递下一个接受请求
                    StartAccept(e);
        ...
        private void CloseClientSocket(AsyncUserToken userToken)
        {
            ...
            Log4Debug(String.Format("Client {0} Disconnection!", userToken.ConnectSocket.RemoteEndPoint.ToString()));
            try
            {
            ...
            {
                userToken.ConnectSocket.Shutdown(SocketShutdown.Receive);
            }
